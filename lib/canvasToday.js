...
const replicateApiKeyIdentifier = process.env.REPLICATE_API_KEY.substring(0, 6);
const inititateImageGeneration = async ({ prompt, seed, headline }) => {
  const replicateResp = await sendReplicateData({
    prompt,
  ...
  return addArt({
    ...
    selected: false,
    votes: 0,
    apiKeyIdentifier: replicateApiKeyIdentifier, // Use this to count and know when we are close to limit
  });
};
