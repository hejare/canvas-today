...
import { createPrompt, getReplicateImage, getUrlResponse, sendReplicateData } from "@/lib/replicateData";
import { addArt, updateArt } from "data/artData";
import { addImageBlock, sendSlackData } from "./slackData";

const replicateApiBaseUrl = process.env.REPLICATE_API_BASE_URL + process.env.REPLICATE_API_MODEL;
...
export const inititateImageGeneration = async (headline) => {
  ...
  const imageIdentifier = replicateResp.urls.get.replace(replicateApiBaseUrl + "/", "");
...
export const updateArtsWithoutImageUrl = async (arts) => {
  const results = await Promise.all(arts.map(art => getUrlResponse(art.imageIdentifier)));

  return await Promise.all(results.map(({ imageUrl, seed }, idx) => {
...
export const postDailySlackPost = async ({ imageUrl, headline, storedMetadata }) => {
  let blocks = [{
    type: "header",
    text: {
      type: "plain_text",
