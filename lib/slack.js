...
export const SLACK_MAX_HEADLINE_LENGTH = 75;
...
const voteBlockMultiSelect = (headlines) => {
  const options = headlines.map(({ headline, id }) => ({
    text: {
      type: "plain_text",
      text: headline,
    },
  ...
  return {
    ...
    text: {
      type: "mrkdwn",
      text: "*Pick Heaadlines from the list:*",
    },
    accessory: {
      type: "multi_static_select",
      placeholder: {
        type: "plain_text",
        text: "Select headlines",
      },
      options: options,
      action_id: ACTION_UPVOTE_HEADLINE,
    },
  };
};
...
export const postVoteHeadlineAction = async (headlines) => {
  ...
  blocks.push(
    voteBlockMultiSelect(
      headlines.filter((h) => h.headline.length <= SLACK_MAX_HEADLINE_LENGTH),
      // .slice(0, 10),
    ),
  );
  blocks.push(
  ...
  // This is for the "voteBlock" function:
